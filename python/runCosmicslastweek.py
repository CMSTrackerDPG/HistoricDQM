...
Epochs = ['LastWeek']
Recos  = ['Prompt']
PDs    = ['Cosmics']      ##other examples: 'SingleMu','DoubleMu'
...
addplots=''
Plots=[' -C cfg/trendPlotsTracking.ini ',' -C cfg/trendPlotsPixel_General.ini']
#Plots=[' -C cfg/trendPlotsTracking.ini ',' -C cfg/trendPlotsPixel_General.ini',' -C cfg/trendPlotsStrip_APVShots.ini']
title=['Tracking_plots','Pixel_plots']
indexOut=['index_tracker.htm','index_pixel.htm']
cosType='ALL'
    ...
    for reco in Recos:
		...
		for icos in range(0,2):
		  ...
		  subprocess.Popen(rmCmd, shell=True).wait()
                  ...
                  print "Making new path:",outPath[:-1]
                  if not os.path.exists(outPath[:-1]): 
                      print "Didn't exist before:",outPath[:-1]
                      os.makedirs(outPath[:-1])
                      print "Now it should",outPath[:-1]
		  else:
                      print "Oh, it is there",outPath[:-1]
                      print "Now it's gone",outPath[:-1]
                      os.makedirs(outPath[:-1])
                      print "Now it's back",outPath[:-1]
