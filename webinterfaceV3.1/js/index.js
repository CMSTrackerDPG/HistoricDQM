		...
		if (dataSet === "StreamExpress" || dataSet === "ZeroBias")
			$.ajax({
				...
				success: function (data) {
					run_fill = data["Run_LHCFill.json"]
								.map(x => ({run: parseInt(x['run']), lhcfill: x['lhcfill']}));
				}
			});
		update_collections();
...
$(document).ready(
	function() {
		...
		show_fills = document.getElementById("show-fills").checked;
		$("#show-fills").change(function() {
			console.log("fills toggled.");
			show_fills = this.checked;
			if (chart_list) {
				chart_list.update_fills();
			}
		});
