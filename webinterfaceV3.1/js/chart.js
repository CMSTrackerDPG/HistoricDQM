class Chart {
    ...
    update(range = null) {
        ...
        if (this.corr) { //correlation plot changes 1/8/17
            ...
            this.bands = [];
            var fills = Array(xValues[0].length);
            if (dataSet === "StreamExpress" || dataSet === "ZeroBias") {
                ...
                for (var i = 0; i < xValues[0].length; i++) {
                    while (curr < run_fill.length && run_fill[curr].run < xValues[0][i])++curr;
		    if (curr == run_fill.length) {
			console.log("Could not find the following runs in the fills file:");
		        console.log(xValues[0].slice(i));
			break;
		    }
         	    if (run_fill[curr].run === xValues[0][i]) {
                        fills[i] = run_fill[curr].lhcfill;
                        if (run_fill[curr].lhcfill !== last_fill) {
                            if (flag) {
                                this.bands.push({
                                    color: "#e6eaf2",
                                    from: start_i - 0.5,
                            ...
                            flag = !flag;
                            last_fill = run_fill[curr].lhcfill;
                        }
                    }
                }
            }
            this.highcharts_options = {
                ...
                xAxis: {
                    ...
                    , plotBands: show_fills ? this.bands.slice() : []
                },
                yAxis: {
                    title: {
                        text: yTitle,
