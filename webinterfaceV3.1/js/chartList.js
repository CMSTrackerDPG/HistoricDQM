class ChartList {
    constructor(dataset, charts) {
        ...
        charts.forEach(function (c) { 
            ...
            self.charts.push(new Chart(c, i++));
        });
...
//	        console.log("Show chart name = " + chart.name);
    ...
    update_fills() {
        this.charts.forEach(function(chart) {
            if (!chart.hidden && chart.chart_obj && chart.bands) {
                var axis = chart.chart_obj.xAxis[0];
                var bands = chart.bands;
                if (show_fills) {
                    for (var i = 0; i < bands.length; i++) {
                        axis.addPlotBand(bands[i]);
                    }
                }
                else {
                    axis.removePlotBand("fills");
                }
            }
        });
    }
