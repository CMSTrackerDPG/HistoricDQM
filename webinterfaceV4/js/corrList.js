	...
	console.log(charts);
        charts.forEach(function (c,idx) { 
            ...
            if (c.corr && c.files.length == 2)
                self.charts.push(new CorrPlot(c, i++,urlList[idx]))
            else
                self.charts.push(new ScatterPlot(c, i++,urlList[idx]));
        });
        this.need_update = true;
        this.page = charts.length;
        this.per_page = charts.length;
